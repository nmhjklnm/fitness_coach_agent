...
采用 **3周阶梯循环（建立/持续/恢复）** + **快碳黄金窗口** + **每周4-5练（有氧0-2次/周，灵活）** + **AI动态调整**，以高碳+高蛋白、温和赤字实现保代谢、保肌肉的高质量减脂。
...
🟩 有氧  🟦 无氧  🟨 混合  ⬜ 休息  ⬛ 未开始
（训练周：周二至周六训练，周日+周一休；本轮从周日（预启动）开始，正式计周从周二起）
```
...
# 2. 创建个人配置文件
cp 档案/个体信息.example.md 档案/个体信息.md
cp 饮食/偏好.example.md 饮食/偏好.md
cp 运动/偏好.example.md 运动/偏好.md

# 3. 填入真实数据（身高、体重、体脂率、训练偏好、饮食限制等）
# 4. 记录健身房设备（参考 运动/设备/健身房A/）
# 5. 让 AI 根据个人信息制定周计划并开始执行
```

**隐私保护**：`档案/个体信息.md`、`饮食/偏好.md`、`运动/偏好.md` 与 `档案/体测表/` 均已在 `.gitignore` 中排除，不会被提交。
...
| 路径 | 说明 |
|------|------|
| `档案/个体信息.md` | 个体信息与全局策略（私密，未入库） |
| `档案/个体信息.example.md` | 个体信息模板 |
| `档案/体测表/` | 定期体测与对比照片 |
| `计划/运动/第XX周-运动.md` | 每周运动课表（动作、负重、目标肌群、RPE） |
| `计划/饮食/第XX周-饮食.md` | 每周饮食方案（宏量、三餐结构、快碳窗口） |
| `饮食/食物.md` | 食材营养数据库 |
| `运动/设备.md` | 可用器械总览与规则 |
| `运动/设备/` | 器械清单与图片（如 `健身房A/`） |
...
| **训练频率** | 练六休一 | **每周4-5练（休2-3天）** | 降低疲劳，稳定恢复 |
| **热量** | 1350-1550 kcal | **≈ 1800-2100 kcal（训练日）** | 保代谢，避免肌肉流失 |
| **碳水** | 95-115g（快碳35g） | **180-220g（含快碳50-60g）** | 糖原充足，力量更稳 |
...
```
训练前30分钟：戗面馒头33g（1/3个）或香蕉1根（≈17-20g快碳）
训练后30分钟：蛋白粉+馒头67g（2/3个）/香蕉1根（≈34-40g快碳）→ 糖原恢复3-5倍速
每训练日用1个完整馒头（100g），简单高效
```
...
- **训练策略**：每周4-5练（周二-周六为主要训练窗；周日/周一休），无氧3-4次+有氧0-1次，≤40-45分钟/次，重质不重量  
- **饮食策略**：高碳水+高蛋白；取消极限期，采用3周阶梯循环；训练日≈1800-2100 kcal；蛋白≥180g、碳水180-220g
